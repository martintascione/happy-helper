    ...
    <div className="flex min-h-screen bg-[#F7F5F1] text-foreground font-sans overflow-x-hidden relative">
      ...
      <main className="flex-1 flex flex-col relative min-h-screen max-w-full overflow-x-hidden">
        ...
        <div className="lg:hidden fixed bottom-10 left-0 right-0 px-8 z-[100]">
          <nav className="h-16 glass-card rounded-full shadow-pill flex items-center justify-around px-3 relative border border-white/60 ring-1 ring-black/5">
                  ...
                  <div key="admin-group" className="relative" ref={adminMenuRef}>
                    <button
                      ...
                      className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 ${
                        anyAdminActive ? "bg-black text-white shadow-lg scale-110" : "text-slate-400 hover:text-slate-600 hover:bg-black/5"
                      }`}
                    >
                      <item.icon size={24} strokeWidth={2} />
                    </button>
                    {isAdminMenuOpen && (
                      <div className="absolute bottom-20 right-0 glass-card rounded-[2rem] shadow-2xl border border-white/60 p-3 min-w-[180px] animate-in fade-in zoom-in-95 slide-in-from-bottom-6 duration-300 ring-1 ring-black/5">
                        {item.items?.map(sub => (
                          <Link
                            key={sub.to}
                            to={sub.to}
                            onClick={() => setIsAdminMenuOpen(false)}
                            className={`flex items-center gap-3 px-4 py-4 rounded-2xl text-[11px] font-black uppercase tracking-wider transition-all ${
                              location.pathname === sub.to ? "bg-black text-white shadow-md scale-[1.02]" : "text-slate-400 hover:bg-black/5"
                            }`}
                          >
              ...
              return (
                <Link
                  ...
                  className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-500 ${
                    isActive ? "bg-black text-white shadow-lg scale-110" : "text-slate-400 hover:text-slate-600 hover:bg-black/5"
                  }`}
                >
                  <item.icon size={24} strokeWidth={2} />
                </Link>
